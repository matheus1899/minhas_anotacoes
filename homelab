=== Instalando Ubuntu Server 26.04 (pela segunda vez) ===
Não usei a versão minimalista. Não instalei nada alem do sistema, nem SSH. Faremos tudo manualmente.
Instalar com rede conectada facilita bastante a vida. Na primeira trouxe mais problemas como não obter IPV4 da rede, 
por isso tive que mexer no arquivo yaml de configuração do netplan (/etc/netplan/00-installer-config.yaml) além de forçar a captura de um IPV4 do linux
...
Então execute o comando: sudo nano /etc/default/console-setup
...
sudo update-initramfs -u //(-u significa update, assim como o -c é create)
sudo reboot

=== SSH ===
sudo apt upgrade //Para atualizar tudo do seu sistema.
Para instalar o SSH use:
sudo apt install -y openssh-server
(-y => Aceita automaticamente todos os avisos)
Para habilitar o serviço, utilize:
sudo systemctl enable ssh
Para executar o serviço utilize: 
sudo systemctl start ssh
Para executar os dois comandos acima de uma vez utilize:
sudo systemctl enable --now ssh

Para configurar o firewall para uso do ssh use:
sudo ufw allow ssh => O retorno foi: [Rules update - Rules update(v6)]
sudo ufw reload    => O retorno foi: [Firewall not enabled (skipping reload)]
...
ip address => Pode-se usar a abreviação que seria: ip a
...
|        dhcp6: true             |        dhcp6: false             |
|        match:                  |                                 |
|          macadress: ...        |      addresses:                 |
|        setname: nome_interface |        - 192.168.1.60/24        |
|    version: 2                  |      routes:                    |
|                                |        - to: default            |
|                                |          via: 192.168.1.1       |
|                                |      match:                     |
...
ssh-keygen -t ed25519 => -t serve pra especificar o tipo de criptografia, ed25519 é o padrão de criptografia utilizado
...
Ao olha o arquivo, observaremos o tipo de criptografia usada, a chave e o usuario_pc@pc
